...
public class Interact : MonoBehaviour
{
    ...
    void Start()
    {
        player = GameObject.FindGameObjectWithTag("Player"); //this is reference by name. Can be used for specifics but not generic things e.g. not Door but player is individual here
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            if (Physics.Raycast(interact, out hitInfo, 10f)) //checks for proximity. takes the line (ray) the output from that line (out raycasthit) and the distance of that line to hit
            {
                ...
                {
                    //dialogue = hit.info check if the dialogue on the hit npc
                    ...
                    if(dialogue != null)
                    {
                        ...
                        dialogue.camLook.enabled = false;
                        dialogue.charLook.enabled = false;
                        dialogue.player.enabled = false;
