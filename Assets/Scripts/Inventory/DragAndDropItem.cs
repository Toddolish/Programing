...
public class DragAndDropItem : MonoBehaviour
{
    ...
    public int slotX, slotY; // size x and size y
    ...
    public void DropItem(int itemID)
    {
        ...
        droppedItem = Resources.Load("Prefabs/" +ItemData.CreateItem(itemID).MeshName) as GameObject;
        // Spawn in the item to the world and remember what item that was
        droppedItem = Instantiate(droppedItem, transform.position + transform.forward * 1, Quaternion.identity);
        // Adding Rigidbody
    ...
    void InventoryDrag(int windowID)
    {
        ...
        for (int y = 0; y < slotY; y++)
        {
            ...
            {
                Rect slotLocation = new Rect(scr.x * 0.125f + x * (scr.x * 0.75f), scr.y * 0.75f + y * (scr.y * 0.65f), 0.75f * scr.x, scr.y * 0.65f);
                GUI.Box(slotLocation, "");
                #region Pickup Item
                /*
                 if we are interacting with left mouse button and the interaction is click down
                 and the mouse cursor is over the item slot while we are not already holding an item and the item
                 slot isnt empty as well as we are not hitting change inventory key left shift
                 */
                if(e.button == 0 && e.type == EventType.MouseDown && slotLocation.Contains(e.mousePosition)&&!isDragging && inventory[i].Name != null && !Input.GetKey(KeyCode.LeftShift))
                {
                    ...
                    isDragging = true;
                    // we remeber where this item come from
                    draggedFrom = i;
                    // debug
                ...
                if (e.button == 0 && e.type == EventType.MouseUp && slotLocation.Contains(e.mousePosition) && isDragging && inventory[i].Name != null)
                {
                    ...
                    // the slot we are dropping is now filled with our dragged item
                ...
                if (e.button == 0 && e.type == EventType.MouseUp && slotLocation.Contains(e.mousePosition) && isDragging && inventory[i].Name != null)
                {
                    Debug.Log("Place: " + draggedItem.Name + "Into: " + inventory[i].Name);
                    // the slot we are dropping hte item into is now filled with the draggedItem
                    inventory[i] = draggedItem;
    ...
    private void OnGUI()
    {
        ...
        if(isDragging)
        {
            if(draggedItem != null)
            {
                ...
                //mouseLocation = ClampToScreen(GUI.Window(2, inventorySize, InventoryDrag, "Drag Inventory"));
            }
        }
        #endregion
    }
